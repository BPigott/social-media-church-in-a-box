...
serve(async (req) => {
  ...
  try {
    ...
    type Call = { key: string; promise: Promise<SpecialistResult> };
    const calls: Call[] = [];
    ...
    if (hasSocialMedia) {
      ...
      for (const platform of validPlatforms) {
        ...
        for (let i = 0; i < Math.max(1, postsPerPlatform); i++) {
          calls.push({ key: `social:${platform}:${i}`, promise: spec(brief, ctx) });
        }
      }
      calls.push({ key: "executiveSummary", promise: runExecutiveSummarySpecialist(brief, ctx) });
    }
    if (hasBibleStudy) calls.push({ key: "bibleStudyGuide", promise: runBibleStudySpecialist(brief, ctx) });
    if (hasDevotional) calls.push({ key: "devotional", promise: runDevotionalSpecialist(brief, ctx) });
    if (hasPodcastDescription) calls.push({ key: "podcastDescription", promise: runPodcastSpecialist(brief, ctx) });

    console.log(`Dispatching ${calls.length} specialist calls in parallel`);
    ...
    for (let i = 0; i < settled.length; i++) {
      ...
      if (call.key.startsWith("social:")) {
        const [, platform, idxStr] = call.key.split(":");
        const idx = parseInt(idxStr);
        if (postsPerPlatform <= 1) {
          generatedContent[platform] = r.value.text;
        } else {
          generatedContent[platform] = generatedContent[platform] ?? [];
          generatedContent[platform][idx] = r.value.text;
        }
      } else {
